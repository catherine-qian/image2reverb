...
def main():
    ...
    parser.add_argument("--d_threshold", type=float, default=None,
                        help="Value over which discriminator weights will be updated by optimizer.")
    ...
    train_dataset = torch.utils.data.DataLoader(train_set, shuffle=True, num_workers=args.num_worker, pin_memory=cuda,
                                                batch_size=args.batch_size)
    val_dataset = torch.utils.data.DataLoader(val_set, num_workers=args.num_worker,
                                              batch_size=args.batch_size)  # For now, to test

    # Main model
    model = Image2Reverb(args.encoder_path, args.depthmodel_path, d_threshold=args.d_threshold, t60p=not args.no_t60p)
    if args.loadckpt is not None or args.loadckpt == 'None':
        print('load from pre-trained checkpoint')
        m = torch.load(args.loadckpt, map_location=model.device)
